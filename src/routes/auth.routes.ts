...
router.get(
  ...
  passport.authenticate("google", {
    failureRedirect: "http://34.72.237.89:3000/home?error=google",
    session: false,
  }),
  (req, res) => {
    ...
    if (info?.message === "alreadyExists" || info?.message === "loginWithGoogle") {
      ...
      return res.redirect(
        `http://34.72.237.89:3000/home?googleAutoLogin=true&token=${info.token}&email=${info.email}`
      );
    }
    ...
    return res.redirect(
      `http://34.72.237.89:3000/home?googleComplete=true&token=${token}&email=${user.email}`
    );
  }
);
